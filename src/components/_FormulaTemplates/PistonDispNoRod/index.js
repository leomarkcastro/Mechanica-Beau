...
function FormulaPistonDispNoRod(props){
    ...
    useEffect(() => {
        ...
        let b = false
        if (inp1 || inp2 || inp3){
            String.raw`V_D = \frac{\pi D^2LN}{2} - \frac{\pi d^2LN}{4}`
            a = String.raw`V_D = \frac{\pi \times ${inp1 ? inp1*inp1 : 'D'} \times ${inp2 ? inp2 : 'L'} \times ${inp3 ? inp3 : 'N'}}{2} - \frac{\pi \times ${inp4 ? inp4*inp4 : 'd'} \times ${inp2 ? inp2 : 'L'} \times ${inp3 ? inp3 : 'N'}}{4}`
            setFormulaData()
        }
        if (inp1 && inp2 && inp3){
            b = String.raw`V_D = ${numberWithCommas((Math.PI*inp1*inp1*inp2*inp3)/2 - (Math.PI*inp4*inp4*inp2*inp3)/4)}`
        }
        setFormula(a)
        setAnswer(b)
    }, [inp1, inp2, inp3, inp4])
...
export default FormulaPistonDispNoRod