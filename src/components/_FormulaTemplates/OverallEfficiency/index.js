...
function FormulaOverallEfficiency(props){
    ...
    useEffect(() => {
        ...
        let b = false
        if (inp1 || inp2){
            a = String.raw`e = \frac{${inp1 ? inp1 : 'P_w'}}{${inp2 ? inp2 : 'P_i'}}`
            setFormulaData()
        }
        if (inp1 && inp2){
            b = String.raw`e = ${numberWithCommas((inp1/inp2))}`
        }
        setFormula(a)
        setAnswer(b)
    }, [inp1, inp2])
...
export default FormulaOverallEfficiency