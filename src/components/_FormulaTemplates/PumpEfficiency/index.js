...
function FormulaPumpEfficiency(props){
    ...
    useEffect(() => {
        ...
        let b = false
        if (inp1 || inp2){
            a = String.raw`e_p = \frac{${inp1 ? inp1 : 'P_w'}}{${inp2 ? inp2 : 'P_b'}}`
            setFormulaData()
        }
        if (inp1 && inp2){
            b = String.raw`e_p = ${numberWithCommas((inp1/inp2))}`
        }
        setFormula(a)
        setAnswer(b)
    }, [inp1, inp2])
...
export default FormulaPumpEfficiency