...
function FormulaDischarge(props){
    ...
    useEffect(() => {
        ...
        let b = false
        if (inp1 || inp2 || inp3){
            a = String.raw`D = \frac{2*${inp1 ? inp1 : 'A'}*${inp2 ? inp2 : 'L'}*${inp3 ? inp3 : 'N'}}{60}`
            setFormulaData()
        }
        if (inp1 && inp2 && inp3){
            b = String.raw`D = ${numberWithCommas((2*inp1*inp2*inp3)/60)}`
        }
        setFormula(a)
        setAnswer(b)
    }, [inp1, inp2, inp3])
...
export default FormulaDischarge