...
function FormulaSlip(props){
    ...
    useEffect(() => {
        ...
        let b = false
        if (inp1 || inp2){
            a = String.raw`S = ${inp1 ? inp1 : 'Q_th'} - ${inp2 ? inp2 : 'Q_a'}`
            setFormulaData()
        }
        if (inp1 && inp2){
            b = String.raw`D = ${numberWithCommas(inp1-inp2)}`
        }
        setFormula(a)
        setAnswer(b)
    }, [inp1, inp2])
    ...
    return(
        <div style={{
            ...
            <FormulaInputCardList {...props} displayDesc={clue}>
                    <FormulaInputCard 
                        variable={String.raw`Q_{th} = `} 
                        value={inp1}
                        onChange= {e => setInp1(e.target.value)}
                    />
                    <FormulaInputCard 
                        variable={String.raw`Q_{a} = `} 
                        value={inp2}
