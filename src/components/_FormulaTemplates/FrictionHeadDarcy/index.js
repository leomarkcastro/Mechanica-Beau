...
function FormulaFrictionHeadDarcy(props){
    ...
    useEffect(() => {
        ...
        if (inp1 || inp2 || inp3 || inp4 || inp5){
            a = String.raw`h_f = \frac{${inp1 ? inp1 : 'v'} \times ${inp2 ? inp2 : 'l'} \times ${inp3 ? inp3*inp3 : 'V^2'}}{2 \times ${inp4 ? inp4 : 'g'} \times ${inp5 ? inp5 : 'D'}}`
            setFormulaData()
        }
        if (inp1 && inp2 && inp3 && inp4 && inp5){
            b = String.raw`h_f = ${numberWithCommas(((inp1*inp2*(inp3*inp3))/(2*inp4*inp5)))}`
        }
        setFormula(a)
        setAnswer(b)
...
export default FormulaFrictionHeadDarcy