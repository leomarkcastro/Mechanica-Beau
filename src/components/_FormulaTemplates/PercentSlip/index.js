...
function FormulaPercentSlip(props){
    ...
    useEffect(() => {
        ...
        let b = false
        if (inp1 || inp2){
            a = String.raw`S = \frac{${inp1 ? inp1 : 'V_D'} - ${inp2 ? inp2 : 'Q'}}{${inp1 ? inp1 : 'Q'}} \times 100`
            setFormulaData()
        }
        if (inp1 && inp2){
            b = String.raw`S = ${numberWithCommas((inp1-inp2) / inp1 * 100)}`
        }
        setFormula(a)
        setAnswer(b)
    }, [inp1, inp2])
    ...
    return(
        <div style={{
            ...
            <FormulaCard 
                ...
                formulaList = {[
                    String.raw`\%S = \frac{V_D - Q}{V_D} \times 100`,
                    (formula ? formula : ''),
                    (answer ? answer : '')
                ]}
            />
...
export default FormulaPercentSlip