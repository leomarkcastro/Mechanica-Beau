...
function FormulaVelocityHead(props){
    ...
    useEffect(() => {
        ...
        let b = false
        if (inp1 || inp2){
            a = String.raw`h_v = \frac{${inp1 ? inp1*inp1 : 'V^2'}}{${inp2 ? 2*inp2 : '2g'}}`
            setFormulaData()
        }
        if (inp1 && inp2){
            b = String.raw`h_v = ${numberWithCommas((inp1*inp1) / (2*inp2))}`
        }
        setFormula(a)
        setAnswer(b)
    }, [inp1, inp2])
...
export default FormulaVelocityHead