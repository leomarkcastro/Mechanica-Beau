...
function ActualPumpEfficiency(props){
    ...
    useEffect(() => {
        ...
        let b = false
        if (inp1 || inp2){
            a = String.raw`EFF = \frac{${inp1 ? inp1 : 'F_A'}}{${inp2 ? inp2 : 'F_D'}} \times {100}`
            setFormulaData()
        }
        if (inp1 && inp2){
            b = String.raw`EFF = ${numberWithCommas((inp1/inp2)*100)}`
        }
        setFormula(a)
        setAnswer(b)
    }, [inp1, inp2])
    ...
    return(
        <div style={{
            ...
            <FormulaCard 
                ...
                formulaList = {[
                    String.raw`EFF = \frac{F_A}{F_D} \times 100`,
                    (formula ? formula : ''),
                    (answer ? answer : '')
                ]}
            />
            ...
            <FormulaInputCardList {...props} displayDesc={clue}>
                    <FormulaInputCard 
                        variable={String.raw`F_A = `} 
                        value={inp1}
                        onChange= {e => setInp1(e.target.value)}
                    />
                    <FormulaInputCard 
                        variable={String.raw`F_D = `} 
                        value={inp2}
...
export default ActualPumpEfficiency