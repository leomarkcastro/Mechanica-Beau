...
function ActualPumpPowerP(props){
    ...
    useEffect(() => {
        ...
        let b = false
        if (inp1 || inp2 || inp3){
            a = String.raw`Power = {${inp1 ? inp1 : 'Q'}} \times {${inp2 ? inp2 : String.raw`\gamma`}} \times {${inp3 ? inp3 : 'H'}}`
            setFormulaData()
        }
        if (inp1 && inp2 && inp3){
            b = String.raw`Power = ${numberWithCommas((inp1*inp2*inp3))}`
        }
        setFormula(a)
        setAnswer(b)
    }, [inp1, inp2, inp3])
    ...
    return(
        <div style={{
            ...
            <FormulaCard 
                title="Actual Pump Power (P)"
                {...props}
                exit={() => {setShown(false)}}
                toggleClue={() => {setClue(!clue)}}
                formulaList = {[
                    String.raw`Power = {Q} \times {\gamma} \times {H}`,
                    (formula ? formula : ''),
                    (answer ? answer : '')
                ]}
            />
...
export default ActualPumpPowerP